...
Last edited [Sabah Ul-Hasan]: Jun 25, 2019
...
Tables 1-2          Statistics breakdown (of all figures)       Lines 632 - 693
Supp Fig 3          Correlations to human population density    Lines 695 - 759
...
Fig3b_GIM <- lm(Width_mm~ Mean_SST + Epoch + Mean_SST*Epoch, data = Clean_New)

Fig3c_aov <- aov(Globosity_WoverL ~ Latitude + Epoch + Latitude*Epoch, data = Clean_New)
Fig3c_GIM <- lm(Globosity_WoverL ~ Latitude + Epoch + Latitude*Epoch, data = Clean_New)

Fig3d_aov <- aov(Globosity_WoverL ~ Mean_SST + Epoch + Mean_SST*Epoch, data = Clean_New)
Fig3d_GIM <- lm(Globosity_WoverL ~ Mean_SST + Epoch + Mean_SST*Epoch, data = Clean_New)

# Create a list for all models
Tab1 <- list(Fig3a_aov, Fig3b_aov, Fig3b_GIM, Fig3c_aov, Fig3c_GIM, Fig3d_aov, Fig3d_GIM) 

# Print summary as Table 1
for(i in Tab1){
...
Fig4b_GIM <- lm(Width_mm ~ Ecoregion + Coastal_Proximity + Ecoregion*Coastal_Proximity, data = subset(Clean_New, Epoch=="Holocene")) # significant for all
...
Fig4c_GIM <- lm(Width_mm ~ Ecoregion + Coastal_Proximity + Ecoregion*Coastal_Proximity, data = subset(Clean_New, Epoch=="Anthropocene")) # significant for all
...
Fig4e_GIM <- lm(Globosity_WoverL ~ Ecoregion + Coastal_Proximity + Ecoregion*Coastal_Proximity, data = subset(Clean_New, Epoch=="Holocene")) # significant for all
...
Fig4f_GIM <- lm(Globosity_WoverL ~ Ecoregion + Coastal_Proximity + Ecoregion*Coastal_Proximity, data = subset(Clean_New, Epoch=="Anthropocene")) # no significance between ecoregions

Tab2 <- list(Fig4a_aov, Fig4b_aov, Fig4b_GIM, Fig4c_aov, Fig4c_GIM, Fig4d_aov, Fig4e_aov, Fig4e_GIM, Fig4f_aov, Fig4f_GIM) 
