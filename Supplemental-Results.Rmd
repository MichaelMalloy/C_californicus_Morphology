...
Raw_original <- read.csv(text = getURL("https://raw.githubusercontent.com/MichaelMalloy/C_californicus_Morphology/master/SupplementalTable2_Data-Raw.csv"))
...
iNat_new <- read.csv("SupplementalTable2_Data-iNat-Raw.csv", stringsAsFactors = FALSE)
...
transform_iNat_data <- function(iNat_df) {
  ...
  transformed <- data.frame(matrix(ncol = ncol(Raw_original), nrow = nrow(iNat_df)))
  colnames(transformed) <- colnames(Raw_original)
  
  # Fill in the transformed dataframe
  transformed$Specimen_Number <- max(Raw_original$Specimen_Number, na.rm = TRUE) + 
  ...
  transformed$Latitude <- iNat_df$latitude
  transformed$Longitude <- iNat_df$longitude
  
  # Convert length and width from the iNat data (appear to be in different units)
  # Based on your original script, iNat data had different measurement units
  # You'll need to calibrate these conversions based on your actual data
  transformed$Length_mm <- iNat_df$length / 10  # Example conversion - ADJUST AS NEEDED
  transformed$Width_mm <- iNat_df$width / 10    # Example conversion - ADJUST AS NEEDED
  
  # Calculate derived metrics (following original script logic)
  transformed$Geometric_Mean <- sqrt(transformed$Length_mm * transformed$Width_mm)
  transformed$Globosity_WoverL <- transformed$Width_mm / transformed$Length_mm
  
  # Set epoch to Anthropocene for iNaturalist observations
  transformed$Epoch <- "Anthropocene"
  
  # Parse observation date
  obs_date <- as.Date(iNat_df$date, format = "%m/%d/%y")
  transformed$Year <- as.numeric(format(obs_date, "%Y"))
  transformed$Decade <- floor(transformed$Year / 10) * 10
  transformed$Month <- as.numeric(format(obs_date, "%m"))
  
  # Assign season based on month (following original script convention)
  transformed$Season <- case_when(
    transformed$Month %in% c(12, 1, 2) ~ "Winter",
    transformed$Month %in% c(3, 4, 5) ~ "Spring",
    transformed$Month %in% c(6, 7, 8) ~ "Summer",
    transformed$Month %in% c(9, 10, 11) ~ "Fall",
    TRUE ~ NA_character_
  )
  
  # Determine county based on latitude (simplified - you may need more precise geocoding)
  transformed$County <- case_when(
    transformed$Latitude >= 38.0 ~ "Sonoma",
    transformed$Latitude >= 37.5 ~ "San Mateo",
    transformed$Latitude >= 36.0 ~ "Monterey",
    transformed$Latitude >= 35.5 ~ "San Luis Obispo",
    transformed$Latitude >= 34.5 ~ "Santa Barbara",
    transformed$Latitude >= 34.0 ~ "Ventura",
    transformed$Latitude >= 33.5 ~ "Los Angeles",
    transformed$Latitude >= 33.0 ~ "Orange",
    transformed$Latitude >= 32.5 ~ "San Diego",
    TRUE ~ "Baja California"
  )
  
  # Determine ecoregion based on location
  transformed$Ecoregion <- ifelse(
    transformed$Latitude > 34.5, 
    "Northern California", 
    "Southern California Bight"
  )
  
  # Set coastal proximity (you may need actual coastal distance calculations)
  transformed$Coastal_Proximity <- "Coastal"  # Default assumption for iNat observations
  
  # Set other fields to NA that will need to be populated from other sources
  na_columns <- c("Site", "SoCal_Group", "Island_Size", "Formation", 
                  "Mean_SST", "Mean_SST_Sdev", "Aperture_mm", "Thickness_mm",
                  "Habitat", "Depth", "BRY", "BAR", "LMP", "TWS", "COR", 
                  "ALG", "SCR", "BOR_TAP", "BOR_STR", "BOR_INC",
                  "mean_pvolc3", "sd_pvolc3", "zooplankton_n_points_used",
                  "zooplankton_mean_distance", "fish_total_catch_wt_kg",
                  "fish_cpue_kg_per_ha_der", "sd_total_catch_wt_kg",
                  "sd_cpue_kg_per_ha_der", "fish_n_points_used",
                  "fish_mean_distance", "Population_Density",
                  "Population_Density_NoBaja", "MPA", "Image_url")
...
# Combine original and new iNaturalist data
Raw_combined <- bind_rows(Raw_original, iNat_transformed)
...
Clean <- Raw_combined %>%
  ...
  filter(!(Width_mm < 15 & Thickness_mm > 2)) %>%
  filter(ifelse(is.na(Aperture_mm), TRUE, Aperture_mm < 6)) %>%
  filter(!(Length_mm > 20 & Aperture_mm < 2)) %>%
  filter(Width_mm > 2) %>%
...
# ---------------------------
# 8. UPDATE YOUR R MARKDOWN SCRIPT
# ---------------------------

cat("\nTo use the integrated data in your RMarkdown script, replace:\n")
cat("Raw <- read.csv(text=getURL(\"https://raw.githubusercontent.com/.../SupplementalTable2_Data-Raw.csv\"))\n")
cat("With:\n")
cat("Raw <- read.csv(\"SupplementalTable2_Data-Raw-Integrated.csv\")\n")
cat("\nOr update the GitHub URL if you upload the integrated file.\n")
```
